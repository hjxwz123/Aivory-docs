...
- [高级运行时变量](/docs/deployment/environment-advanced)：全部 `AIVORY_*` 运行时调优、限流、RAG、队列和管理任务变量。
- [沙盒变量](/docs/deployment/environment-sandbox)：全部 `SANDBOX_*` sidecar 与会话运行时变量。
- [前端构建变量](/docs/deployment/environment-frontend)：全部 `VITE_*` 变量。
...
沙盒资源、会话、上传和工作区变量见[沙盒变量](/docs/deployment/environment-sandbox)。模型渠道、搜索、OCR 和 embedding 的运营步骤见[渠道、模型与策略](/docs/admin/channels-models)和[知识库、RAG 与存储](/docs/admin/knowledge-rag)。
