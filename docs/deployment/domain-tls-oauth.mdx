...
In every case, never expose PostgreSQL, Redis, Qdrant, or `sandbox` through the proxy. They should stay only on the Docker internal network or a private network. If you front Aivory with Cloudflare (proxied DNS or a Tunnel), see the [Cloudflare Proxy guide](./cloudflare-proxy.mdx) for SSE streaming, upload-size, and origin-TLS specifics.
