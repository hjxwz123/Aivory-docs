...
无论哪种方式，都不要通过代理暴露 PostgreSQL、Redis、Qdrant 或 `sandbox`。它们只应留在 Docker 内部网络或私有网络中。若使用 Cloudflare（橙色云代理或 Tunnel），SSE 流式响应、上传体积限制与回源 TLS 的额外注意事项见 [Cloudflare 代理](./cloudflare-proxy.mdx)。
