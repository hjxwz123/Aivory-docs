...
sidebar_position: 1
description: 'Entry point of the authoritative Aivory data-layer reference: how the 57 tables are stored, how migrations run, how to back up and restore, and common operational SQL.'
---

# Database overview and operations

This section is the complete reference for Aivory v2.4.6's relational data model: **57 tables and 667 effective columns, every one documented**. The authoritative schema is embedded in the Go binary:

- SQLite (personal/dev): `server/internal/store/schema.sql` (1,087 lines, `//go:embed`, `store.go:22-23`)
- PostgreSQL (full edition): `server/internal/store/schema_pg.sql` (1,017 lines, `store.go:25-26`)

Both files declare **the same 57 tables** (identical column names and semantics; only dialect types differ), applied idempotently at startup by `store.Migrate()`. The final effective schema = embedded `CREATE TABLE` + ~130 additive `ALTER TABLE ADD COLUMN` statements in `store.Migrate` (a few columns exist only in ALTERs — e.g. `users.group_expires_at`, `messages.feedback`, `conversations.workspace_id`; they are marked **※ ALTER-added** in the text).
...
| --- | --- | --- |
| [Users & authentication](./users-auth.mdx) | 5 | `users`, `refresh_tokens`, `login_histories`, `oauth_providers`, `oauth_identities` |
...
| [Workspaces & members](./workspaces-members.mdx) | 6 | `workspaces`, members, invites, policies, per-library permissions, audit log |
...
Totals: 5+6+9+7+15+6+7+2 = **57 tables**. Aivory has no `schema_migrations`-style bookkeeping table (see [migration mechanism](#migration-there-is-no-external-migration-tool) below), so there is nothing else to place.
...
![Aivory database overview: 57 tables grouped by domain with major foreign keys and the storage-engine split between editions](/img/db-schema-overview.svg)

## Where data lives in each edition

Aivory ships a single Go backend binary; the storage engine is chosen at boot by `DATABASE_URL` / `REDIS_URL` / `QDRANT_URL` (see [Deployment and environment](../../deployment/environment.mdx)). **No table is personal-edition-only** — all 57 tables are created in both editions; only vectors, cache, and the queue differ in engine:

| Data category | Personal (SQLite) | Full (PostgreSQL + Redis + Qdrant) |
| --- | --- | --- |
| All 57 relational tables | single file `DATA_DIR/aivory.db`, WAL (`DATABASE_URL=/app/data/aivory.db?_pragma=journal_mode(WAL)&_pragma=busy_timeout(5000)`, `deploy/docker-compose.personal.yml:28`) | named volume `pgdata` of the `postgres` container (`postgres:16-alpine`) |
...
| Admin runtime config (model policy, RAG knobs, quotas, moderation, announcement — full key list in [System & audit](./system-audit.mdx)) | [`settings`](./system-audit.mdx) table (DB key/value, hot reload) | same; across replicas via Redis channel `cfg:invalidate` |
