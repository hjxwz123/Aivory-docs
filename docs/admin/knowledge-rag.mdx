...
Either way the 61-table schema is identical: even Full creates `vector_points` but keeps it empty, so logical backups and migration tooling stay engine-agnostic. Domain bindings and passkey credentials are relational rows in the same database and are included in the normal backup flow.
