...
## Unreleased — workspace announcements

The current implementation adds a **Workspace menu → Members → Announcement** editor for workspace owners and administrators, with an HTML popup, optional image, five-second required-reading delay, and an independently enabled top bar. Members and guests see notices for the active workspace; dismissal is stored in the browser, with no server-side read receipt. See [Workspace announcements](../admin/workspace-announcements.mdx).

The new `workspace_announcements` table brings both database schemas to **61 tables**. Startup migration creates it automatically; each save records `announcement.updated` and publishes `workspace.announcement_updated`. Full backups include its rows, uploaded images require files to be included, and configuration-only exports exclude the rows. Compatible older archives without this new table restore with no workspace announcement configured. These notes describe development functionality and do not imply it is included in the `v2.4.7` image.
...
- **Email-domain enrollment and access restrictions**: platform administrators map an exact email domain to a workspace from the separate `/admin/domains` page. New eligible registrations are enrolled transactionally; optional personal-space locks prevent ordinary bound users from switching, creating, joining, or directly addressing personal resources. Per-user overrides can restore personal access. Platform administrators retain a personal space. See [Domain enrollment and workspace access](../admin/domain-management.mdx).
- **Workspace administration**: `/admin/workspaces` can create a workspace with an existing active user as owner/administrator. The workspace detail view can transfer ownership only to a current member; the previous owner becomes a regular member for this platform-admin operation.
- **Conversation and project operations**: users can archive conversations in bulk and manage project conversations from the project action menu; clearing personal conversations preserves workspace isolation.
...
The domain feature adds `registration_domains` and `domain_users` to both database dialects. These tables are included in backup ordering, use `ON DELETE RESTRICT` for workspace references and `ON DELETE CASCADE` for user/rule bindings, and are covered in the [database reference](../reference/database/overview.mdx). Existing users are not retroactively enrolled; changing an email does not remove an existing binding.
