...
sidebar_position: 2
description: Column-by-column reference for users, sessions, OAuth identities, and passkeys, covering 2FA, session families, and WebAuthn login.
---

# Users and authentication tables

Six identity-domain tables (type notation defined in the [overview conventions](./overview.mdx); **※ ALTER-added** = exists only via `store.Migrate`'s ALTER loop, not in `schema.sql`'s CREATE TABLE).
...
- [Domain enrollment and workspace access](../../admin/domain-management.mdx) — how domain bindings relate to users and workspaces
