...
| --- | --- | --- |
| Personal edition | The entire `DATA_DIR` | Contains `aivory.db`, embedded vectors, uploads, artifacts, local objects, passkey credentials, domain rules, and backup archives |
| Full edition | `pgdata`, `redisdata`, `qdrantdata`, `sandbox-archives`, `DATA_DIR` | Relational data (including passkeys and domain bindings), vectors, cache persistence, and durable workspaces and files must stay consistent |
...
### Workspace announcement backup coverage

Builds containing [workspace announcements](../admin/workspace-announcements.mdx) create `workspace_announcements` automatically at startup. Full archives include this table and workspace audit records. Select **include files** to preserve uploaded announcement images in `UPLOAD_DIR/icons/`; a database-only archive retains image URLs but does not copy image bytes. External image URLs still depend on their external host.

Configuration export/import excludes workspace announcement rows and does not migrate or overwrite a workspace's notice. Although that archive can include shared icon files, those files alone do not recreate the announcement configuration.

An otherwise valid, supported historical backup may omit this newly added table. Restore leaves it empty, so no workspace announcement is shown until an administrator saves one. Full restore replaces target data and does not retain target-only announcements. Browser-local dismissal records are not part of either archive.

After restore, enter a workspace as a member and check the popup, top bar, and image. Use a fresh browser profile to avoid confusing a previously remembered dismissal with missing restored data.
