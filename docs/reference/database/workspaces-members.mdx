...
sidebar_position: 7
description: Column-by-column reference for workspaces, workspace_members, workspace_invites, workspace_policies, workspace_kb_member_permissions, and workspace_audit_logs.
---

# Workspace and member tables

Six workspace-domain tables. Before reading, remember one rule: **business tables belong to a workspace via a soft-reference `workspace_id` column** (`''` = personal data) with no foreign key; the tables here own "the workspace itself, membership, policy, and audit" (notation per the [overview](./overview.mdx)).
