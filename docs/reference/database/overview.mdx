...
sidebar_position: 1
description: 'Entry point of the authoritative Aivory data-layer reference: how the 60 tables are stored, how migrations run, how to back up and restore, and common operational SQL.'
---
...
| [Workspaces & members](./workspaces-members.mdx) | 8 | `workspaces`, members, invites, policies, per-library permissions, audit log, `registration_domains`, `domain_users` |
...
Totals: 6+6+9+7+15+8+7+2 = **60 tables**. Aivory has no `schema_migrations`-style bookkeeping table (see [migration mechanism](#migration-there-is-no-external-migration-tool) below), so there is nothing else to place.
...
![Aivory database overview: 60 tables grouped by domain with major foreign keys and the storage-engine split between editions](/img/db-schema-overview.svg)

## Where data lives in each edition

Aivory ships a single Go backend binary; the storage engine is chosen at boot by `DATABASE_URL` / `REDIS_URL` / `QDRANT_URL` (see [Deployment and environment](../../deployment/environment.mdx)). **No table is personal-edition-only** — all 60 tables are created in both editions; only vectors, cache, and the queue differ in engine:

| Data category | Personal (SQLite) | Full (PostgreSQL + Redis + Qdrant) |
| --- | --- | --- |
| All 60 relational tables | single file `DATA_DIR/aivory.db`, WAL (`DATABASE_URL=/app/data/aivory.db?_pragma=journal_mode(WAL)&_pragma=busy_timeout(5000)`, `deploy/docker-compose.personal.yml:28`) | named volume `pgdata` of the `postgres` container (`postgres:16-alpine`) |
