...
工作空间公告存储在 [`workspace_announcements`](./workspaces-members.mdx#workspace_announcements)，与 `settings.announcement` 分开。其 JSON 还包含独立顶部通知栏的 `bar_enabled`、`bar_html` 和 `bar_updated_at`，全局公告也支持这三个属性。工作空间公告保存时记录审计动作 `announcement.updated`，并发布实时事件 `workspace.announcement_updated`。
