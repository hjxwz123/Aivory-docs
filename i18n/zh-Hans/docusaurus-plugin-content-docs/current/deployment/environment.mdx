...
| 变量 | 适用范围 | 默认值 | 作用与取值 | 敏感性 |
| --- | --- | --- | --- | --- |
| `SEARCH_PROVIDER` | 所有部署 | 空 | `duckduckgo`（或 `ddg`）、`searxng`、`serper`、`brave`、`tavily` 或 `auto`。DuckDuckGo 需明确选择，但无需其他配置；`auto` 不会选择它。 | 普通 |
| `SEARCH_API_KEY` | Serper/Brave/Tavily | 空 | 搜索服务商 API Key。DuckDuckGo 和自托管 SearXNG 不需要。 | 密钥 |
