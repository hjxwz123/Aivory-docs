...
Aivory v2.4.6 and later. Relevant screens: "Capabilities & integrations → Documents" (RAG and embedding settings), "System → Storage & uploads", and the user-facing knowledge bases under `/kb`. Screenshots show the Chinese admin UI; English labels are cited inline.
...
Either way the 57-table schema is identical: even Full creates `vector_points` but keeps it empty, so logical backups and migration tooling stay engine-agnostic.
