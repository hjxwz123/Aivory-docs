...
![Aivory login page: brand panel on the left, email and password form on the right with remember-password, forgot-password, and sign-up links](/img/auth-login-page.png)
...
Set `ALLOWED_ORIGINS=https://chat.example.com` in `.env.personal` for any domain or HTTPS reverse proxy. Full details on ports, reverse proxies, OAuth callbacks, and multi-domain configuration are in [Domains, HTTPS, and OAuth](../deployment/domain-tls-oauth.mdx).
