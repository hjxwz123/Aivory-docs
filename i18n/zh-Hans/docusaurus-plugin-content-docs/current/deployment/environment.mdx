...
常规的文多多 AI PPT 配置应使用管理后台。只有对应数据库设置为空时，以下变量才作为回退；
已经保存的管理后台值优先。这些变量由 API 进程启动时读取，修改后需要重建 API 容器。

| 变量 | 默认值 | 用途 | 敏感性 |
| --- | --- | --- | --- |
| `DOCMEE_API_KEY` | 空 | 服务端签发用户范围 iframe token 的文多多 API Key。 | 密钥 |
| `DOCMEE_API_BASE_URL` | `https://docmee.cn` | 服务端 token API 地址。 | 内部网络 |
| `DOCMEE_DOMAIN` | 空 | iframe 域名，例如国际版端点或受控代理。 | 内部网络 |
| `DOCMEE_SDK_URL` | 固定的 Docmee SDK | 浏览器 SDK 脚本地址；仅指向受信任的镜像或自托管副本。 | 内部网络 |
| `DOCMEE_SDK_BASE_URL` | 空 | 浏览器经由受控代理访问时的 SDK API base URL。 | 内部网络 |

浏览器无需获得 API Key。功能门控、积分结算和服务商部署选项见[AI PPT（文多多 Docmee）](../admin/ai-ppt.mdx)。
