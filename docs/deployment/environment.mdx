...
| Variable | Applies to | Default | Purpose and values | Sensitivity |
| --- | --- | --- | --- | --- |
| `SEARCH_PROVIDER` | All deployments | Empty | `duckduckgo` (or `ddg`), `searxng`, `serper`, `brave`, `tavily`, or `auto`. DuckDuckGo is explicitly selected and needs no other setting; `auto` does not choose it. | Normal |
| `SEARCH_API_KEY` | Serper/Brave/Tavily | Empty | Search provider API key. DuckDuckGo and self-hosted SearXNG do not need one. | Secret |
