...
本页列出服务端的高级调优变量。它们都不是首次部署的必填项；除非你正在处理明确的性能、容量、限流或兼容性问题，否则应保留默认值。基础部署、认证、存储、向量、集成和 Compose 变量见[环境变量总览](/docs/deployment/environment)。沙盒与前端构建变量分别见[沙盒变量](/docs/deployment/environment-sandbox)和[前端构建变量](/docs/deployment/environment-frontend)。
