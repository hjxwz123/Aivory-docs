...
The official application image serves both the web UI and `/api` from a single process. This means that in a normal deployment the browser and the API are always same-origin: the reverse proxy only needs to forward the entire request to the application — no separate frontend deployment, no frontend API address configuration, and no CORS setup for a single domain.
...
## CORS and a Split Frontend

The official image has no required setting such as `PUBLIC_ORIGIN`. As long as the web UI and the API are served by the same `app`, the application works on whichever valid domain the user entered through.

`ALLOWED_ORIGINS` is only for cases where you deliberately place the frontend on another origin and have the browser access the API cross-origin. Fill in an exact list of `scheme://host[:port]` values, for example:

```dotenv
ALLOWED_ORIGINS=https://console.example.com,https://staging-console.example.com
```

Do not replace a controlled domain list with broad wildcards. A normal monolithic deployment does not need to set it.
