...
title: 工作空间与成员表
sidebar_position: 7
description: workspaces、workspace_members、workspace_invites、workspace_policies、workspace_kb_member_permissions、workspace_audit_logs 六表的逐列参考。
---

# 工作空间与成员表

工作空间域 6 张表。理解本域先记住一条：**业务表的归属靠 `workspace_id` 软引用列**（`''` = 个人数据），该列没有外键；本页的表只负责"空间本身、成员、策略与审计"（记法约定见[总览](./overview.mdx)）。
