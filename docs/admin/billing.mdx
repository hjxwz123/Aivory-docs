...
### AI PPT fixed-price credits

The optional Docmee **AI PPT** workbench is configured in the same Credits and
quotas page, but it does not use model token pricing. It reserves and settles a
fixed number of credits per completed deck, using the provider's presentation ID
to prevent duplicate charges. The sidebar entry stays hidden until an
administrator supplies a key and enables the integration. See [AI PPT
(Docmee)](./ai-ppt.mdx) for setup, visibility, provider endpoints, and the
