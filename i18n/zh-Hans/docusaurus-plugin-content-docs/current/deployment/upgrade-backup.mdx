...
包含[工作空间公告](../admin/workspace-announcements.mdx)功能的构建会在启动时自动创建 `workspace_announcements` 表。完整备份包含此表及工作空间审计记录。需要保留 `UPLOAD_DIR/icons/` 下的上传配图时，勾选**包含文件**；仅备份数据库会保留图片 URL，不复制图片文件。外部图片仍依赖原来的图片服务。

配置导入导出不包含工作空间公告数据行，因此不会迁移或覆盖工作空间的公告。虽然配置归档可以包含共享图标文件，但仅有图片不能重建公告配置。

其他条件合法且版本受支持的历史备份，可以缺少这张新增表。恢复后此表为空，管理员保存公告前不会显示工作空间通知。完整恢复替换目标数据，不保留目标环境独有的公告。浏览器本地的关闭记录不属于任何一种归档。

恢复后用成员账号进入工作空间，检查弹窗、顶部通知栏与配图。可使用新的浏览器配置文件，避免将之前记住的关闭状态误认为恢复数据缺失。
