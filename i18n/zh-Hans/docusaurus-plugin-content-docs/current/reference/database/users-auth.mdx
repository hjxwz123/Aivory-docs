...
title: 用户与认证表
sidebar_position: 2
description: users、refresh_tokens、login_histories、oauth_providers、oauth_identities 五表的逐列参考，含 2FA、会话家族与第三方登录。
---

# 用户与认证表

本页覆盖身份域 5 张表（列类型记法见[总览页的命名与类型约定](./overview.mdx)；**※附加列** = 只由 `store.Migrate` 的 ALTER 添加、不在 `schema.sql` 的 CREATE TABLE 中）。
